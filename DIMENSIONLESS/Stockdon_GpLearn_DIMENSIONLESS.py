...
from mda_Char2 import MaxDiss_Simplified_NoThreshold
from mda_Char2 import MaxDiss_Simplified_NoThresholdNOR
from mda_Char2 import DeNormalize
...
## ============ Part 3: Transforming Data to DIMENSIONLESS ===================

dataDLESS = pd.DataFrame()
dataDLESS ['n'] = data['n']/data['Hs0']
# dataDLESS ['Hs0'] = data['Hs0'] #Since I'm dividing everything for Hs0, Hs0/Hs0 =1
dataDLESS ['Tp'] = data['Tp']**2*(9.8/data['Hs0'])**0.5
dataDLESS ['tanB'] = data['tanB'] #Nondimensional
dataDLESS ['d50'] = (data['d50']/1000)/data['Hs0'] #D50 (mm) to (m)
dataDLESS ['L0'] = data['Hs0']/data['L0'] #Wave steepness
dataDLESS ['Irr'] = data['Irr'] #Nondimensional

## ====================== Part 4: MDA (Test Data)  ===========================

dataset = dataDLESS [['n', 'Tp', 'tanB', 'd50', 'L0', 'Irr']]

# variables to use
vns = ['n', 'Tp', 'tanB', 'd50', 'L0', 'Irr']

# subset size and scalar index
n_subset = 150            # subset size ~30% data
ix_scalar = [0, 1, 2, 3, 4, 5]        # n, Hs0, Tp, tanB, d50

#MDA return denormalize data - TRAIN and TEST set
trainDENOR, testDENOR  = MaxDiss_Simplified_NoThreshold(dataDLESS[vns].values[:], n_subset, ix_scalar)
trainDENOR = pd.DataFrame(data=trainDENOR, columns=vns)
testDENOR = pd.DataFrame(data=testDENOR, columns=vns)

#MDA return normalize data - TRAIN and TEST set ##Return test indices
trainNOR, testNOR, minis, maxis, indices_test = MaxDiss_Simplified_NoThresholdNOR(dataDLESS[vns].values[:], n_subset, ix_scalar)
...
## =================== Part 5: Test and Trainset Data ========================    

X_train = trainNOR [['Tp', 'tanB', 'd50', 'L0', 'Irr']] #NORMALIZED
y_train = trainDENOR ['n'] #DENORMALIZED
X_test = testNOR [['Tp', 'tanB', 'd50', 'L0', 'Irr']] #NORMALIZED
y_test = testDENOR ['n']  #DENORMALIZED

## ======================= Part 6: Set Functions ==============================
...
feature_names = ['Tp', 'tanB', 'd50', 'L0', 'Irr']
...
## ============= Part 8: Visualize Predicted X Real Setup =============

Hs0_test = data['Hs0'][indices_test]; Hs0_test = Hs0_test.reset_index(drop=True)
y_test_predicted = est_gp.predict(X_test)*Hs0_test #Return Setup to original values (n*Hs0) for prediction
y_test2 = y_test*Hs0_test

plt.figure(2)
plt.plot(y_test2, '*k', label = 'Tested'); plt.legend(loc = 'best')
...
plt.figure(3)
plt.plot(y_test2, y_test_predicted, '*k')
plt.xlabel ('Real Setup (m)')
plt.ylabel ('Predicted Setup (m)')
plt.title('GpLearn')

n_predd = np.squeeze(y_test_predicted)
# z = np.polyfit(y_test2, n_predd, 1)
# p = np.poly1d(z)
# plt.plot(y_test2,p(y_test2),"r")
# plt.show
# print ("y=%.2fx+(%.2f)"%(z[0],z[1]))

## Starting from zero
x = np.array(y_test2) [:, np.newaxis]
...
## ============= Part 9: Compare with Stockdon =============

Tp_test = data['Tp'][indices_test]; Tp_test = Tp_test.reset_index(drop=True)
tanB_test = data['tanB'][indices_test]; tanB_test = tanB_test.reset_index(drop=True)

#Stockdon Formula: 0.35*tanB*(Hs0*L0)**0.5
L0 = 1.56*(Tp_test**2)
n_stock = 0.35*tanB_test*(Hs0_test*L0)**0.5
...
plt.figure(7)
plt.plot(y_test2, n_stock, '*c', label = 'Stockdon'); plt.legend(loc = 'best')
plt.plot(y_test2, y_test_predicted, '*r',  label = 'Predicted'); plt.legend(loc = 'best')
plt.xlabel ('Real Setup (m)')
...
print('Coefficient of determination - Predicted: %.2f'
      % r2_score(y_test2, y_test_predicted))

print('Coefficient of determination - Stockdon: %.2f'
      % r2_score(y_test2, n_stock))